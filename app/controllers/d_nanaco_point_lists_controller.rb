...
class DNanacoPointListsController < ApplicationController
  ...
  def search
    ...
    if params[:start_ymd] == nil or params[:start_ymd] == ""
      @start_ymd = (Time.now - 60*60*24).strftime("%Y/%m/%d")
    else
    ...
    shops.each do |shop|
      ...
      @num = Date.parse(@to_ymd) - Date.parse(@from_ymd)
        ...
        for i in 0..@num.to_i
          ...
          nanaco_point_nums = get_nanaco_point_num(result_date,shop.id)
          nanaco_points = get_nanaco_point(result_date,shop.id)
          row.item(:shop_name).value(shop.shop_ryaku) 
          unless nanaco_points == nil
            row.item("p1_#{i+1}").value(nanaco_point_nums.nanacopoint_num.to_i)
            p1_sum_col = p1_sum_col + nanaco_point_nums.nanacopoint_num.to_i
            row.item("p2_#{i+1}").value(nanaco_points.nanacopoint.to_i)
            p2_sum_col = p2_sum_col + nanaco_points.nanacopoint.to_i
          end
        end #for
